...
`internal/api/protocollo.go` (attesa del claim 20 s, massimo 25; online = contatto entro 40 s; battito mai più
...
| `crea_bozza_outlook` | «Rispondi» (capacità `bozze`) | `PayloadCreaBozza{…, bozza_id, testo}` | `Reply()`, inserisce il testo sopra la firma, `UserProperties["CockpitBozza"]=bozza_id` (idempotenza), `Save()`, mai `Send()` (`consenti_invio=false`) | `RisultatoBozza{entry_id}` → `bozza.stato='aperta'` |

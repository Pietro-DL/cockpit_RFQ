...
// PostgreSQL vero. Un nodo «52920000_PRT» di un cliente che dichiara «_PRT» e' il pezzo 52920000; un
// componente accettato come «52920000_PRT» prima che la regola ci fosse e' lo stesso pezzo, e il nodo ne
...
// La famiglia dei disegni 529 senza revisione nel codice. La regex non chiude con \b: in «52920000_PRT»
// trova 52920000.
const (
	famiglia529Semplice      = `{"famiglie_codice": [{"regex": "(?P<codice>529\\d{5})", "descrizione": "disegni 529", "esempio": "52920000"}]`
	famiglia529ConSuffisso   = famiglia529Semplice + `, "suffissi_decorativi": ["_PRT"]}`
	famiglia529SenzaSuffissi = famiglia529Semplice + `}`
)
...
// stepConSuffissi: un assieme 53040000 con due figli scritti come li scrive il CAD del cliente.
func stepConSuffissi() fattiSTEP {
	return fattiSTEP{nodi: []string{"#1=53040000", "#2=52920000_PRT", "#3=52930000_C_PRT"}, archi: []string{"#1>#2*2", "#1>#3"}}
}

// Un nodo dello STEP che porta il suffisso decorativo del cliente propone il codice del pezzo: 52920000, non
// 52920000_PRT; con la revisione prima del suffisso, codice e revisione separati. Il grezzo del file resta
...
func TestUnNodoConIlSuffissoDecorativoProponeIlCodiceDelPezzo(t *testing.T) {
	...
	a := b.allegatoStep("53040000.stp", strings.Repeat("d", 64))
	b.applica(a, stepConSuffissi().json())
	if got, want := b.letturaDeiNodi(), "#1=53040000/-/generico:aperta #2=52920000/-/generico:aperta #3=52930000/C/generico:aperta"; got != want {
		t.Fatalf("nodi = %q, attesi %q", got, want)
	}
	if got := uno[string](b, `SELECT nome_grezzo || '|' || id_grezzo FROM componente_proposta WHERE thread_id = $1 AND chiave = '#2'`, b.thread); got != "52920000_PRT|52920000_PRT" {
	...
	if !strings.Contains(msg, "52920000 entra nella BOM come sciolto") {
		t.Errorf("messaggio: %q", msg)
	}
	if got := b.codiciDeiComponenti(); got != "52920000" {
		t.Errorf("componenti = %q, atteso il solo 52920000", got)
	}
}

// Un pezzo accettato come «52920000_PRT» prima che la regola ci fosse e' lo stesso pezzo che lo STEP, letto
// adesso con la regola, chiama 52920000: il nodo ne e' un duplicato, agganciato a lui, e l'arco uguale a quello
...
func TestUnComponenteConIlSuffissoAccettatoPrimaDellaRegolaEDuplicatoDelNodo(t *testing.T) {
	for _, nodo := range []string{"52920000", "52920000_PRT"} {
		t.Run("nodo "+nodo, func(t *testing.T) {
			b := nuovoBanco(t)
			p := b.componente("53040000", db.TipoComponenteFinito)
			x := b.componente("52920000_PRT", db.TipoComponenteSciolto)
			b.arco(p, x, 2)
			b.conRegole(regoleSuffissoPRT)
			a := b.allegatoStep("53040000.stp", strings.Repeat("e", 64))
			es := b.applica(a, fattiSTEP{nodi: []string{"#1=53040000", "#2=" + nodo}, archi: []string{"#1>#2*2"}}.json())
			if got, want := b.nodiEComponenti(), "#1:duplicato:"+p.String()+" #2:duplicato:"+x.String(); got != want {
				t.Errorf("nodi = %q, attesi %q", got, want)
			}
			if got := uno[string](b, `SELECT codice FROM componente_proposta WHERE thread_id = $1 AND chiave = '#2'`, b.thread); got != "52920000" {
				t.Errorf("il nodo propone %q: il codice letto e' quello del pezzo, 52920000", got)
			}
			...
			if got := b.codiciDeiComponenti(); got != "52920000_PRT 53040000" {
...
// Il controllo: senza la regola lo STEP si legge come prima. Il nodo tiene il suffisso nel codice, e 52920000
// non e' il componente «52920000_PRT»: e' un nodo nuovo, con il suo arco, da decidere.
func TestSenzaSuffissiLoStepSiLeggeComePrima(t *testing.T) {
	t.Run("i codici dei nodi", func(t *testing.T) {
		b := nuovoBanco(t)
		a := b.allegatoStep("53040000.stp", strings.Repeat("d", 64))
		b.applica(a, stepConSuffissi().json())
		if got, want := b.letturaDeiNodi(), "#1=53040000/-/generico:aperta #2=52920000_PRT/-/generico:aperta #3=52930000_C_PRT/-/generico:aperta"; got != want {
			t.Errorf("nodi = %q, attesi %q", got, want)
		}
	})
	t.Run("il componente con il suffisso", func(t *testing.T) {
		...
		b.arco(p, x, 2)
		a := b.allegatoStep("53040000.stp", strings.Repeat("e", 64))
		es := b.applica(a, fattiSTEP{nodi: []string{"#1=53040000", "#2=52920000"}, archi: []string{"#1>#2*2"}}.json())
...
// «52920000_PRT» contiene il codice di famiglia 52920000 ma non lo e': la radice dello STEP lo corregge, con
...
func TestD16IlCodiceDelDocumentoRestaSoloSeEUnCodiceDiFamiglia(t *testing.T) {
	casi := []struct {
		...
		{"il codice con il suffisso, con la regola", famiglia529ConSuffisso, "52920000_PRT", "52920000:-:regola_cliente:disegni 529:id:52920000_PRT"},
		{"il codice con il suffisso, senza la regola", famiglia529SenzaSuffissi, "52920000_PRT", "52920000:-:regola_cliente:disegni 529:id:52920000_PRT"},
		{"un codice di famiglia", famiglia529ConSuffisso, "52922757", "52922757:-:nome_file:-:-:-"},
	}
	for _, c := range casi {
		t.Run(c.nome, func(t *testing.T) {
			...
			b.applica(a, fattiSTEP{nodi: []string{"#1=52920000_PRT", "#2=53011111"}, archi: []string{"#1>#2"}}.json())
			...
			if got := b.letturaDeiNodi(); !strings.HasPrefix(got, "#1=52920000/-/famiglia:aperta") {
...
// Una famiglia con la revisione in coda non legge il suffisso come revisione: «52920000_PRT» e' 52920000 senza
// revisione, non 52920000 rev P; «52930000_C_PRT» e' 52930000 rev C.
func TestUnaFamigliaConLaRevisioneNonLeggeIlSuffissoComeRevisione(t *testing.T) {
	b := nuovoBanco(t)
	b.conRegole(`{"famiglie_codice": [{"regex": "(?P<codice>529\\d{5})(?:_(?P<rev>[A-Z]))?", "rev_nel_codice": true, "descrizione": "disegni 529",
		"esempio": "52922757_B"}], "suffissi_decorativi": ["_PRT"]}`)
	a := b.allegatoStep("52940000.stp", strings.Repeat("a", 64))
	b.applica(a, fattiSTEP{nodi: []string{"#1=52940000", "#2=52920000_PRT", "#3=52930000_C_PRT"}, archi: []string{"#1>#2", "#1>#3"}}.json())
	if got, want := b.letturaDeiNodi(), "#1=52940000/-/famiglia:aperta #2=52920000/-/famiglia:aperta #3=52930000/C/famiglia:aperta"; got != want {
		t.Errorf("nodi = %q, attesi %q", got, want)
	}
}

// Il pezzo nato come «52920000_PRT» e poi archiviato: il nodo «52920000» dice che accettarlo lo ripristina, e
// accettarlo ripristina proprio quello (nessun secondo componente 52920000).
func TestAccettareIlNodoRipristinaIlPezzoConIlSuffisso(t *testing.T) {
	...
	b.conRegole(regoleSuffissoPRT)
	a := b.allegatoStep("53040000.stp", strings.Repeat("b", 64))
	b.applica(a, fattiSTEP{nodi: []string{"#1=53040000", "#2=52920000"}, archi: []string{"#1>#2*2"}}.json())
	if got := uno[string](b, `SELECT stato || ':' || coalesce(nota, '') FROM componente_proposta WHERE thread_id = $1 AND chiave = '#2'`, b.thread); !strings.HasPrefix(got, "aperta:") ||
		!strings.Contains(got, "52920000_PRT") {
	...
	if !strings.Contains(msg, "52920000_PRT era archiviato: ripristinato") {
		t.Errorf("messaggio: %q", msg)
	}
	if got := b.codiciDeiComponenti(); got != "52920000_PRT 53040000" {
...
// Un file «52920000» arriva in una RFQ in cui il pezzo e' nato come «52920000_PRT» prima della regola: il piano
// non chiede di aggiungere 52920000 (sarebbe un secondo componente), dice che e' lo stesso pezzo e offre di
// assegnarlo a quello.
func TestIlPianoRitrovaIlPezzoConIlSuffisso(t *testing.T) {
	...
	pdf, _ := b.allegatoInStaging(msg, 1, "52920000_PRT.pdf", "%PDF disegno")
	b.esegui(`UPDATE allegato SET stato = 'analizzato' WHERE allegato_id = $1`, pdf)
	b.esegui(`UPDATE documento_proposta SET tipo_proposto = 'disegno_2d', codice = '52920000', fonte = 'cartiglio', confidenza = 95 WHERE allegato_id = $1`, pdf)
	...
	// senza la regola il pezzo non si ritrova: 52920000 non e' nella BOM
	b.conRegole(`{}`)
	if err := b.tx(func(q *db.Queries) (err error) {
		p, err = fascicolo.LeggiPianoFascicolo(b.ctx, q, b.thread)
		return err

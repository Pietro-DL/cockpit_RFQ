...
| `rfq/documenti` | il **fascicolo** di una RFQ. `path.go`: i nomi sul NAS (`NomeSicuro`, `CartellaThread`, `PathDocumento`, `NomeFileSicuro`) — percorsi sempre RELATIVI alla radice, con `\` come separatore; il prefisso long-path sta in `platform/storage/nas`. `integrita.go`: il **ricognitore** che confronta `documento` con i file veri e scrive `nas_anomalia`, e `AllineaDocumento` (che su un conflitto rifiuta). `copia_nas.go`: che cosa SIGNIFICA copiare un documento sul NAS (`CopiaSulNas`) e dove se ne ritrova il contenuto (`SorgenteStaging`); `ripresa.go`: il contenuto sparito dalla cache che si riprende da solo (`RiprendiContenuto`); `cartella_thread.go`: la cartella di una RFQ (`CreaCartellaThread`). Chi le chiama e' l'esecutore, che sta in `app/runtime` | sì |

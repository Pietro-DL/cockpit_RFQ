...
| `POST /proposta/{id}/conferma` · `/scarta` | `documento` + `copia_nas`. Con `nas_scrittura` spenta il documento resta `in_coda`. Un CAD 3D, disegno 2D o DXF senza codice non si conferma: errore esplicito e proposta aperta (A2.2). Il documento agganciato prende il codice del componente così com'è scritto | `core/inbox/classificazione`, `platform/coda`, `platform/storage/nas` |

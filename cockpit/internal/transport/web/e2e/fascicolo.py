...
class Banco:
    ...
    def conferma_nell_editor(self, apri):
        ...
        verifica(ed.locator(".bomed-albero .bomed-riga", has_text="77811111").count() == 1, "77811111 non e' nell'albero dell'editor")
        ...
        verifica(self.avviso().startswith("Struttura di 77722757 confermata"), "avviso dopo l'editor: %r" % self.avviso())
...
def prova_a(b):
    ...
    verifica(carta.count() == 1, "la card del nodo nuovo 77811111, proposta dallo STEP, non c'e'")
...
def prova_b(b):
    ...
    verifica(scheda.is_visible() and "77817189" in scheda.inner_text(), "il dettaglio del componente scelto non si vede")
    ...
    verifica(b.page.locator("#anteprima .in-arrivo", has_text="77817189.pdf").count() == 1, "il PDF del particolare non e' fra i file in arrivo")
    ...
    verifica(b.page.locator("#file-%s" % b.a.pdf).count() == 1, "il PDF 77817189.pdf non e' fra i candidati")
    verifica(b.viva(), "la pagina si e' ricaricata")
...
def prova_d(b):
    ...
    verifica(riga.count() == 1, "la proposta del nodo 77811111 non c'e' nella vista Albero")
    ...
    verifica(b.page.locator("#vista .node:not(.proposal)", has_text="77811111").count() >= 1, "il nodo accettato non e' un nodo pieno dell'albero")
    ...
    rimando = b.page.locator('#tela li.li-arco-proposto > div.carta[id^="arco-%s-"]' % b.a.stp, has_text="77811111")
    verifica(rimando.count() == 1, "l'arco proposto 77720517 → 77811111 non e' disegnato sotto l'assieme")
    ...
    pieno = b.carta_confermata("77811111", b.a.assieme)
    verifica(pieno.count() == 1, "dopo la struttura 77811111 non e' una card confermata sotto l'assieme")
    verifica("proposta" not in (pieno.get_attribute("class") or "").split(), "la card di 77811111 e' ancora una proposta")
...
def prova_e(b):
    ...
    verifica(b.viva(), "la pagina si e' ricaricata")
    avviso = b.avviso()
    verifica(avviso == "3 file assegnati al componente 77817189.", "avviso: %r" % avviso)
...
def prova_h(b):
    ...
    riga = b.page.locator("#cassetto #codice-77899999")
    verifica(riga.count() == 1, "il codice nuovo 77899999 non e' nel cassetto")
    b.clic_e_aspetta(riga.get_by_role("button", name="+ Particolare"), "/fascicolo/codice/aggiungi")
    verifica(b.avviso() == "77899999 entra nella BOM come particolare.", "avviso: %r" % b.avviso())
    verifica("nel Fascicolo" in b.page.locator("#cassetto #codice-77899999").inner_text(), "dopo il gesto la riga non dice «nel Fascicolo»")
    ...
    verifica(b.page.locator("#vista table.griglia tr", has_text="77899999").count() == 1, "il particolare aggiunto non e' nella griglia")
    verifica(b.viva(), "la pagina si e' ricaricata")
...
def prova_i(b):
    ...
    verifica(b.viva(), "la pagina si e' ricaricata")
    verifica(b.page.locator('[id="proposta-%s-#3"]' % b.a.stp).count() == 0, "la card proposta di 77811111 e' rimasta")
    verifica(b.carta_confermata("77811111", b.a.assieme).count() == 1, "77811111 non e' nato sotto l'assieme")

...
| `ingest` | un lotto di messaggi → `messaggio`, `messaggio_casella`, `allegato`, `conversazione`, `riferimento_portale`, `proposta_triage`; transazione per lotto con savepoint; scarti; cursore; staging automatico **deciso dal modo del sync** (D30); replay degli scarti; `marcatori.go` (7B): `CockpitRichiestaFornitore` e `CockpitBozza` letti dalla Posta inviata — la richiesta prende la sua mail (`inviata`), la mail entra nella RFQ (è l'unico `thread_id` scritto dall'ingest: non una coincidenza, la nostra firma), la bozza risulta partita; `Motori.PerFornitore`: l'unione delle famiglie dei clienti con richieste aperte a quel fornitore (IB8); `controparte.go` (7A): la **controparte** scritta sul messaggio a ogni ingest (`SetControparteMessaggio`, mai sopra una `manuale`), il **ritriage mirato** per indirizzo/dominio sui soli messaggi non decisi (`Ritriage`), il ricalcolo a lotti all'avvio dei messaggi entrati prima della 0014 (`RicalcolaControparti`) | `workerapi`, `web/censisci.go`, `web` (anagrafica), `main.go` | `db`, `domain`, `aggancio`, `jobs` |
| `aggancio` (7B) | `richieste.go`: i candidati verso una **richiesta a un fornitore** (R0 In-Reply-To/References verso la nostra mail, R1 conversazione, R3f codice di famiglia di un cliente con richieste a quel fornitore) in `candidato_richiesta`; `RichiesteManuali` (RF_oggetto): una nostra mail a un fornitore che cita un identificativo di una RFQ aperta | `ingest` | `db`, `domain` |
| `fornitori` | l'import del seme dei fornitori (`seme.go`, 7A.4): `Leggi` (convalida il file), `Calcola` (anteprima senza scritture: da creare, da aggiungere, presenti, **non risolti**, avvisi), `Applica` (una transazione; scrive solo il risolto; idempotente). Non sovrascrive, non sposta domini, non inventa | `web/fornitori_admin.go` | `db` |
| `domain` | l'**interpretazione** pura, senza DB: `codici.go` (famiglie del cliente, riferimento RFQ, triage con precedenza risposta > candidati > nuova RFQ; **un fornitore, un ambiguo o uno sconosciuto in entrata non produce mai `nuova_rfq`**, per costruzione: 7B.3, D34), `atto.go` (7C.0: l'**atto** — `richiesta_offerta`, `offerta`, `domanda_chiarimento`, `conferma_ricezione`, `non_business`, `incerto`… — e il **legame** — `nuovo`, `risposta`, `aggiornamento`, `nessuno`, `incerto`; prima della 0016 era `intento.go`, con un vocabolario legato alla direzione; `NonRFQ`, `IntentoFornitore`; i candidati verso una richiesta), `catena.go` (taglio della catena di risposta: l'interpretazione legge quello che è stato scritto adesso, blocco 6), `regole.go` (schema di `cliente.regole`, ✓/✗), `controparte.go` (7A, D33: il resolver — interno > contatto esatto > dominio > sconosciuto, in entrambe le anagrafiche = ambiguo, sull'uscita il primo destinatario esterno — su un'interfaccia `Rubrica`; `DominioPubblico`), `convenzioni.go` (7A, D39: suffisso/regex → insieme di lavorazioni con l'evidenza; `ValidaConvenzione` in scrittura, `LeggiConvenzioni` ✓/✗ in lettura, come per le regole), `proposta.go` (tipo del documento da nome ed estensione), `path.go` (nome cartella NAS), `anagrafica.go`, `aggancio.go` (punteggi R1–R5) | `ingest`, `web`, `workerapi` | — |
...
| `web` | la UI: rotte, sessioni, ruoli (`ruoli.go`: consultazione < operatore < tecnico < admin, un solo `soloRuolo`), postazione della sessione, **Inbox a tre quadranti** (Buyer, Fornitori, Da validare; direzione come filtro, 7A/D34) e triage, `censisci.go` («Censisci come fornitore / cliente» dal pannello, con il ritriage mirato), thread (con «Riprova copie»), allegati e conferma, admin (job, scarti, postazioni, anagrafica clienti con `convenzioni_admin.go`, **fornitori** con `fornitori_admin.go` e l'import, **Integrità NAS**), template HTMX | browser | `jobs`, `ingest`, `domain`, `aggancio`, `fornitori`, `nas`, `db` |
...
| `GET /messaggio/{id}/triage`, `POST /messaggio/{id}/rfq` · `/aggancia` · `/ignora` | `triageForm`, `nuovaRFQ`, `agganciaEsistente`, `ignora` | decisioni con `FOR UPDATE` sul messaggio; `nuovaRFQ` crea `thread_offerta`, gli identificativi **selezionati**, e accoda `crea_cartella_thread`; log in `messaggio_aggancio_log`. Il pannello di un messaggio con controparte `fornitore` non ha «Nuova RFQ» (solo «Aggancia a…» e «Ignora») | `domain`, `aggancio`, `jobs` |
| `POST /messaggio/{id}/risposta-fornitore` (richiesta_id), `POST /messaggio/{id}/richiesta-fornitore` (thread_id, fornitore_id, lavorazione) | `rispostaFornitore`, `richiestaFornitoreManuale` (`richieste.go`, 7B) | la prima: «è la risposta a questa richiesta» → aggancio alla RFQ cliente (stesso `agganciaMessaggioAThread`, con il log), `messaggio.richiesta_fornitore_id`, richiesta a `risposta`, proposte aperte sugli allegati (PDF, fogli, documenti) → `offerta_fornitore`. La seconda: «sì, è la richiesta a X per la RFQ Y» su una nostra mail mandata a mano → nasce la richiesta già `inviata` con questa mail, la mail entra nella RFQ; mai un thread nuovo | `richieste.sql` |
| `POST /thread/{id}/richiesta`, `POST /thread/{id}/richiesta/{rid}/annulla` | `nuovaRichiestaFornitore`, `annullaRichiestaFornitore` | la richiesta a un fornitore (bozza) e, con `bozza=1`, la bozza «nuovo» in Outlook: oggetto `RFQ <cliente> <buyer> <codici>`, destinatari = contatti del fornitore, `Marcatori{CockpitRichiestaFornitore}`; capacità `bozze`, postazione della sessione, dalla casella del primo messaggio della RFQ servito da quella postazione. Senza bozza la richiesta nasce lo stesso e la frase dice perché | `jobs`, `api.PayloadCreaBozza` |
| `GET /messaggio/{id}/censisci?come=fornitore\|cliente`, `POST /messaggio/{id}/censisci` | `censisciForm`, `censisci` | operatore (non admin): crea il fornitore (o lo riusa per ragione sociale) o il cliente, con dominio e/o contatto/buyer e le lavorazioni; poi `ingest.Ritriage(indirizzo, dominio)` sui messaggi **non decisi** con quell'indirizzo o dominio; l'avviso riporta i conteggi. Un dominio pubblico (gmail…) sposta il predefinito sull'indirizzo. Senza transazione, di proposito: le scritture non distruttive rispondono con «di chi è» e in una transazione abortita non potrebbero | `ingest/controparte.go`, `domain.DominioPubblico`, `AggiungiDominioFornitore` |
...
| `/admin/anagrafica/*` | `soloAdmin` | clienti (cartella NAS letta da qui, 4A), domini, buyer, fabbisogno, regole (form e JSON), articoli, banco di prova (= la stessa funzione del triage); dalla 7A la sezione **Lavorazioni e fornitori** (`convenzioni_admin.go`): `/{id}/convenzione` (+`/elimina`, `/attiva`), `/{id}/qualifica` (+`/elimina`), `/{id}/lavorazioni/prova`. Aggiungere un dominio o un buyer esegue il ritriage mirato (`ritriagePer`): dalla 0014 la controparte è scritta sul messaggio e senza ricalcolo i messaggi già arrivati resterebbero «sconosciuti» | `domain/regole.go`, `domain/codici.go`, `domain/convenzioni.go`, `ingest/controparte.go` |
| `/admin/fornitori`, `/admin/fornitori/{id}` (+`/dominio`, `/dominio/elimina`, `/contatto`, `/contatto/elimina`, `/lavorazioni`, `/qualifica`, `/qualifica/elimina`), `/admin/fornitori/importa` | `soloAdmin` (`fornitori_admin.go`) | la scheda del fornitore (generale, domini e contatti, lavorazioni e qualifiche, posta); scritture non distruttive come per i clienti (un dominio già di un altro dice di chi è; una ragione sociale doppia non crea un secondo; una capacità con una qualifica sopra non si toglie); l'import del seme con `azione=anteprima\|applica` (file caricato o JSON incollato). Stanno sotto `/admin/fornitori` e non sotto `/admin/anagrafica/fornitori/{id}`: il mux di Go 1.22 considera quel pattern in conflitto con `/admin/anagrafica/{id}/regole` | `fornitori`, `ingest/controparte.go` |
...
| `POST /api/v1/jobs/{id}/result` | verifica il tentativo, `applicaRisultato` per tipo: **sync** → `ultimo_received` sempre, le frontiere (`coperto_fino_a` o `storico_fino_a`) solo per le cartelle dichiarate `completa`; **stage** → promuove `_parti/<allegato>.parte.<token>` in `_contenuti/<ab>/<sha256>.<ext>`, poi accoda `estrai_archivio` (server) se è un archivio, altrimenti `analizza_allegato`; **analisi** → `analisi_fatti(sha256, versione, hash_config)` e una `documento_proposta` per ogni allegato aperto con quello sha256, ciascuno con le regole del proprio cliente (`propostaDaAnalisi`); **bozza** → `bozza`; poi `CompletaJob`/`FallisciJob`. Codice e revisione che arrivano da fuori (nome file, risultato del worker) passano da `codiceRevSicuri` (7C.1): fuori misura → non in colonna, grezzi in `dettagli` (`codice_scartato`, `rev_scartata`) |
| `GET /api/v1/allegati/{id}/contenuto` | 7C.1, P0: i byte di un allegato in staging al SOLO tentativo valido di un job `analizza_allegato` di quell'allegato (`scaricaContenuto`, `contenuto.go`): 409 tentativo non valido, 422 job di un altro allegato, 410 contenuto sparito dalla cache («usa Riscarica»). Il payload dell'analisi non porta più `path_staging`: un percorso sul disco del server non dice niente a un worker su un altro PC |
...
intero**. Le riletture costano una deduplica per Message-ID; i buchi non sono ammessi. Dalla 7A, prima del triage,
l'ingest risolve la **controparte** (`domain.RisolviControparte` con la rubrica in database) e la scrive sul
messaggio (`controparte_tipo`, `_cliente_id`/`_fornitore_id`, `_via`, `_il`); il cliente per il triage e lo staging
automatico è quello della controparte, solo in entrata; con controparte `fornitore` o `ambiguo` il triage non
propone mai `nuova_rfq`.

**Richiesta a un fornitore (7B).** RFQ › «Nuova richiesta a un fornitore» → `richiesta_fornitore` (`bozza`) →
bozza in Outlook con il marcatore → l'operatore preme Invia in Outlook → il sync della Posta inviata porta
`marcatori` nel `MessaggioIn` → `ingest/marcatori.go`: `SetRichiestaInviata`, aggancio alla RFQ, `SetBozzaInviata`.
Mandata a mano: la mail in uscita a un fornitore passa dal triage (`daInterpretare`), `aggancio.RichiesteManuali`
...
o cliente in anagrafica → `ingest.Ritriage(indirizzo, dominio)`: per ogni messaggio **non deciso** (`thread_id`
NULL, nessuna proposta accettata o rifiutata) con quell'indirizzo o dominio ricalcola controparte, triage e
candidati; le decisioni prese non si toccano → il pannello torna con i conteggi.
...
| Strato | Domanda | Dove |
|---|---|---|
| **Fatto** | cosa è arrivato, da chi, quando, con quali allegati e hash; **chi c'è dall'altra parte** secondo l'anagrafica di quel momento (`messaggio.controparte_*`, ricalcolabile); che cosa c'è davvero sul NAS | `ingest`, `jobs/integrita.go`, tabelle `messaggio*`, `allegato`, `analisi_fatti`, `nas_anomalia` |
...
| **Decisione** | cosa l'operatore ha deciso | `web/triage.go`, `web/allegati.go`, `web/thread.go`, `web/richieste.go`, `web/integrita_admin.go`; `thread_id`, `identificativo_thread`, `documento`, `richiesta_fornitore`, `messaggio.richiesta_fornitore_id`, `messaggio_aggancio_log` |
...
| che cosa sta facendo il mittente (atto), a che cosa appartiene la mail (legame), e perché uno sconosciuto non propone una RFQ | `domain/atto.go`, `domain/codici.go:attoELegameCliente`, `triageFornitore`, `triageSconosciuto`, `triageUscita` |
| perché una risposta di un fornitore non chiude la richiesta | `web/richieste.go:rispostaFornitore` (lo stato cambia solo con l'atto confermato: offerta, declinata), `db/queries/richieste.sql:SetRichiestaOffertaRicevuta` |
| che cosa è un soggetto «altro» e perché non è il cestino degli sconosciuti | `domain/controparte.go` (la sesta controparte, `AltroPerRecapito`), `db/queries/altro.sql`, migrazioni 0016–0017 |
| gli invarianti R0/marcatore del contratto di classificazione | `ingest/invarianti_7c_db_test.go` (I4, I5), `ingest/marcatori.go` |
| come una nostra mail a un fornitore si lega alla richiesta | `ingest/marcatori.go` (dal marcatore), `aggancio/richieste.go:RichiesteManuali` e `web/richieste.go:richiestaFornitoreManuale` (a mano) |
| perché l'offerta di un fornitore propone quella richiesta | `aggancio/richieste.go:CalcolaRichieste`, `db/queries/richieste.sql` (R0, R1, R3f) |
| chi è la controparte di un messaggio, e perché un fornitore non apre una RFQ | `domain/controparte.go:RisolviControparte`, `ingest/controparte.go`, `domain/codici.go:Triage` (il ramo `Controparte`) |
| perché un messaggio sta in quel quadrante | la colonna `quadrante` di `v_inbox` (0017: solo dalla controparte), `db/queries/messaggi.sql` (`ListInbox`, `ContaQuadranti` filtrano su quella), `web/web.go:quadranteValido` |
| da un codice pezzo alla lavorazione e ai fornitori qualificati | `domain/convenzioni.go`, `db/queries/fornitori.sql` (`ListFornitoriQualificati`), scheda cliente › Lavorazioni e fornitori › Prova un codice |
| che cosa scrive (e non scrive) l'import dei fornitori | `fornitori/seme.go:calcola` (i non risolti), `Applica` |

...
| che cosa è un messaggio (intento) e perché uno sconosciuto non propone una RFQ | `domain/intento.go`, `domain/codici.go:triageFornitore`, `triageSconosciuto`, `triageUscita` |
...
| perché un messaggio sta in quel quadrante | `db/queries/messaggi.sql` (`ListInbox`, `ContaQuadranti`), `web/web.go:quadranteValido`, `quadranteDi` |
